...
alias rbc='clear;git ls-files -m | xargs ls -1 2>/dev/null | grep '\.rb$' | xargs rubocop'                                               #Before Commit:
alias rbp='clear;git diff-tree -r --no-commit-id --name-only master@\{u\} head | xargs ls -1 2>/dev/null | grep '\.rb$' | grep -v schema | xargs rubocop' #Before Push:
